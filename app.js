  ...
  return `
    <div style="font-size:13px;max-width:260px;">
      ...
      <div style="margin-top:8px;display:flex;gap:6px;flex-direction:column;">
        ...
        ${canEdit ? `<button onclick="renommerClient('${safeId}')" style="background:#009688;color:#fff;border:none;padding:8px;border-radius:6px;cursor:pointer;">?? Modifier</button>
                     <button onclick="supprimerClient('${safeId}')" style="background:#e53935;color:#fff;border:none;padding:8px;border-radius:6px;cursor:pointer;">??? Supprimer</button>` : `<div style="font-size:12px;color:#777;padding-top:4px;">(Modification réservée)</div>`}
      </div>
    </div>
  `;
}
...
async function calculerItineraire(destLat, destLng) {
  ...
  try {
    ...
    if (infoDiv) infoDiv.innerHTML = `?? <b>Distance</b> : ${distanceKm} km — ?? <b>Durée</b> : ${dureeMin} min`;
  } catch (e) {
...
// Quand on clique sur le X → effacer
...
// Afficher ou cacher le X selon le contenu
...
// Ajouter la barre à ton interface
document.querySelector('#searchArea').appendChild(searchContainer);

// --- Ton code existant pour filtrer les clients ---
searchInput.addEventListener('input', (e) => {
